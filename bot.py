...
# --- КОНФИГУРАЦИЯ ---
# Токен считывается из переменных окружения Railway (Environment Variables)
BOT_TOKEN = os.getenv("BOT_TOKEN", "7851053002:AAHYlFzZ7bpT7gknQdSena2FUFbrf8UZ3wg")

# Директории для хранения и обработки биометрии
...
async def cmd_start(message: types.Message, state: FSMContext):
    """Обработка команды /start и отмены действия."""
    await state.clear()
...
async def cmd_about(message: types.Message):
    ...
    await message.answer(
        ...
        "Бот предназначен для поиска информации в публичных источниках (Surface Web), "
...
async def process_photo(message: types.Message, state: FSMContext):
    """Обработка фотографии и биометрический анализ."""
    ...
    try:
        ...
        logging.error(f"Ошибка обработки DeepFace: {e}")
